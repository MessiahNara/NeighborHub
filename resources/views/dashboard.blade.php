<x-app-layout>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-6 pb-24">
                <a href="{{ route('category.show', 'buy-sell') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="absolute top-5 right-5 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg border-2 border-slate-900 z-20">{{ $counts['buy_sell'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'borrow') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    @if($counts['borrow'] > 0)
                    <span class="absolute top-5 right-5 bg-white text-[#36B3C9] text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-[#36B3C9]">{{ $counts['borrow'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'services') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="absolute top-5 right-5 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-slate-900">{{ $counts['services'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'events') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    @if($counts['events'] > 0)
                    <span class="absolute top-5 right-5 bg-white text-[#36B3C9] text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-[#36B3C9]">{{ $counts['events'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'places') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    @if($counts['places'] > 0)
                    <span class="absolute top-5 right-5 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-slate-900">{{ $counts['places'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'announce') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <span class="absolute top-5 right-5 bg-white text-[#36B3C9] text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-[#36B3C9]">{{ $counts['announce'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'complaints') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="absolute top-5 right-5 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-slate-900">{{ $counts['complaints'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'requests') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <span class="absolute top-5 right-5 bg-white text-[#36B3C9] text-xs font-bold px-2 py-1 rounded-full shadow-lg border-2 border-[#36B3C9]">{{ $counts['requests'] }}</span>
                    @endif
                </a>
            </div>
        </div>
    ...
    <script>
        // Get the live post counts from Laravel
        const currentCounts = @json($counts);
        
        // Fetch saved (already viewed) counts from the browser's storage
        let savedCounts = JSON.parse(localStorage.getItem('neighborhub_notifs')) || {};
        
        const badge = document.getElementById('notifBadge');
        const notifList = document.getElementById('notifList');
        
        const categoryNames = {
            'buy_sell': 'Buy & Sell', 'borrow': 'Borrow', 'services': 'Services',
            'events': 'Events', 'places': 'Places', 'announce': 'Announcements',
            'complaints': 'Complaints', 'requests': 'Requests'
        };
        ...
        function renderNotifications() {
            ...
            for (const [category, count] of Object.entries(currentCounts)) {
                const savedCount = savedCounts[category] || 0;
                
                // Safety fix: If a post was deleted, reset local count so it doesn't break future notifs
                if (count < savedCount) {
                    savedCounts[category] = count;
                    localStorage.setItem('neighborhub_notifs', JSON.stringify(savedCounts));
                }

                // Calculate only the "Unread" posts
                const newCount = count - (savedCounts[category] || 0);

                if (newCount > 0) {
                    hasNew = true;
                    const url = `/category/${category.replace('_', '-')}`;
                    
                    // Creates the list item. Clicking it runs markAsRead()
                    html += `
                        <a href="${url}" onclick="markAsRead('${category}', ${count})" class="flex items-center gap-4 p-5 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 group">
                            ...
                            <div>
                                <p class="text-sm font-black text-slate-700 capitalize leading-tight">${categoryNames[category] || category}</p>
                                <p class="text-[10px] font-bold text-[#36B3C9] uppercase tracking-wider mt-0.5">${newCount} New Post${newCount > 1 ? 's' : ''}</p>
                            </div>
                        </a>
                    `;
                }
            ...
            if (hasNew) {
                ...
                notifList.innerHTML = `
                    <div class="p-12 text-center">
                        <div class="bg-slate-50 h-16 w-16 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-200">
                            <i class="fas fa-bell-slash text-xl"></i>
                        </div>
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest leading-snug">All caught up!<br>No new posts.</p>
                    </div>
                `;
            }
        }

        // Initialize notification list and badge on page load
        renderNotifications();

        // Specific Category Click: Mark only that category as read
        window.markAsRead = function(category, count) {
            savedCounts[category] = count;
            localStorage.setItem('neighborhub_notifs', JSON.stringify(savedCounts));
            // Navigation to the category happens immediately after this
        };

        // Clear All Button: Marks everything as read and removes the red dot
        window.clearAllNotifications = function() {
            savedCounts = { ...currentCounts }; // Sync saved storage to current live counts
            localStorage.setItem('neighborhub_notifs', JSON.stringify(savedCounts));
            renderNotifications(); // Updates the UI instantly to empty
        };
