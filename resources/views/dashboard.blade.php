<x-app-layout>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="relative z-50 flex flex-col md:flex-row justify-between items-end md:items-center mb-12 gap-6">
                <div>
                    ...
                    <p class="text-slate-500 mt-2 font-semibold text-sm">Binmaley | NeighborHub Dashboard</p>
                </div>
                    ...
                    <div class="relative inline-block text-left">
                        ...
                        <div id="notificationTab" class="hidden absolute right-0 mt-3 w-80 bg-white rounded-[2.5rem] shadow-2xl ring-1 ring-black ring-opacity-5 transition-all transform origin-top-right overflow-hidden border border-slate-100">
                            ...
                            <div class="p-6 border-b border-slate-100 flex justify-between items-center">
                                <div>
                                    <h3 class="text-base font-black text-slate-800 uppercase tracking-tight leading-none">Updates</h3>
                                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-1.5">NeighborHub</p>
                                </div>
                                <button onclick="clearAllNotifications()" class="text-xs font-bold text-red-400 bg-red-50 hover:bg-red-100 px-3 py-2 rounded-xl transition-colors shadow-sm">
                                    Clear
                                </button>
                            </div>
                            ...
                            <div class="p-4 bg-slate-50 text-center border-t border-slate-100">
                                <button onclick="toggleNotifications()" class="text-xs font-bold text-slate-400 uppercase tracking-wide hover:text-[#36B3C9] transition-colors">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-6 pb-24">
                <a href="{{ route('category.show', 'buy-sell') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="font-black text-white text-base relative z-10">Buy & Sell</span>
                    @if(!empty($counts['buy_sell']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['buy_sell'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'borrow') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <span class="font-black text-white text-base">Borrow</span>
                    @if(!empty($counts['borrow']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['borrow'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'services') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="font-black text-white text-base relative z-10">Services</span>
                    @if(!empty($counts['services']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['services'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'events') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <span class="font-black text-white text-base">Events</span>
                    @if(!empty($counts['events']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['events'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'places') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="font-black text-white text-base relative z-10">Places</span>
                    @if(!empty($counts['places']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['places'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'announce') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <span class="font-black text-white text-base">Announcements</span>
                    @if(!empty($counts['announce']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['announce'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'complaints') }}" class="relative group bg-slate-900 p-8 rounded-[2.5rem] shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center overflow-hidden">
                    ...
                    <span class="font-black text-white text-base relative z-10">Complaint</span>
                    @if(!empty($counts['complaints']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['complaints'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('category.show', 'requests') }}" class="relative group bg-[#36B3C9] p-8 rounded-[2.5rem] shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <span class="font-black text-white text-base">Request File</span>
                    @if(!empty($counts['requests']))
                    <span class="absolute top-4 right-4 bg-white text-slate-800 text-xs font-bold px-2.5 py-1 rounded-full shadow-lg z-20">{{ $counts['requests'] }}</span>
                    @endif
                </a>
            </div>
        </div>
    ...
    <script>
        ...
        const chatNotifications = @json($chatNotifications ?? []);
        ...
        function renderNotifications() {
            ...
            if (chatNotifications && chatNotifications.length > 0) {
                ...
                chatNotifications.forEach(msg => {
                    ...
                    html += `
                        <a href="${url}" class="flex items-center gap-4 p-5 bg-white hover:bg-slate-50 transition-colors border-b border-slate-100 border-l-4 border-l-[#36B3C9] group">
                            <div class="h-10 w-10 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform ${iconClass}">
                                <i class="fas ${iconClass.split(' ')[0]} text-sm"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-bold text-[#36B3C9] uppercase tracking-wide mb-1">Message</p>
                                <p class="text-sm font-semibold text-slate-700 leading-snug truncate">${mainText}</p>
                                <p class="text-xs text-slate-400 mt-0.5 truncate">${postTitle}</p>
                            </div>
                        </a>
                    `;
                });
                ...
                recentUpdates.forEach(post => {
                    ...
                    if (!readNotifs.includes(notifKey)) {
                        ...
                        if (['requests', 'complaints'].includes(post.category)) {
                            ...
                            statusBadge = `<span class="ml-2 px-2 py-0.5 rounded text-xs font-bold uppercase ${badgeClass}">${post.status || 'Pending'}</span>`;
                        }
                        ...
                        html += `
                            <a href="${url}" onclick="markAsRead('${notifKey}')" class="flex items-center gap-4 p-5 hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0 group">
                                <div class="h-10 w-10 rounded-2xl bg-[#36B3C9]/10 flex items-center justify-center text-[#36B3C9] flex-shrink-0 group-hover:scale-110 transition-transform">
                                    <i class="fas ${config.icon} text-sm"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-bold text-[#36B3C9] uppercase tracking-wide mb-1 flex items-center flex-wrap gap-1">
                                        ${config.name} ${statusBadge}
                                    </p>
                                    <p class="text-sm font-bold text-slate-700 leading-snug truncate">${post.title}</p>
                                    <p class="text-xs text-slate-400 mt-0.5"><i class="fas fa-user mr-1"></i> ${authorText}</p>
                                </div>
                            </a>
                        `;
                    }
            ...
            if (hasNew) {
                ...
                notifList.innerHTML = `
                    <div class="p-12 text-center opacity-70">
                        ...
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-wide leading-snug">All caught up!<br>No new updates.</p>
                    </div>
                `;
            }
        }
        ...
        window.clearAllNotifications = function() {
            if (recentUpdates) {
                recentUpdates.forEach(post => {
                    const timeToTrack = isAdmin ? post.created_at : post.updated_at;
                    const notifKey = post.id + '_' + new Date(timeToTrack).getTime();
                    if (!readNotifs.includes(notifKey)) readNotifs.push(notifKey);
                });
                localStorage.setItem('neighborhub_read_notifs', JSON.stringify(readNotifs));
            }
